...
| Go 1.22+ | сборка агента-сборщика (`agents\go`) | только для пересборки агента |
...
| Prometheus | `localhost:9090` | скрейп метрик агента-сборщика (см. §5) |
...
## 5. Агент-сборщик (онлайн-режим)

Слежение за растущим техжурналом с доставкой в ClickHouse в реальном времени —
агент `agents\go\tj-agent-go.exe` (победитель bake-off). Полное руководство по
развёртыванию: [deploy/agent/README.md](../deploy/agent/README.md).

**Конфигурация** — YAML-файл (эталон с комментариями:
[agents/go/tj-agent.example.yaml](../agents/go/tj-agent.example.yaml)):

```yaml
inputs:
  - 'C:\1C_TJ\logs'                # каталоги ТЖ (рекурсивно, *.log)
sink: 'clickhouse://localhost:9001/tj?schema=rich&table=events'
state_dir: 'C:\ProgramData\tj-agent\state'   # чекпоинты (ноль потерь)
metrics: '0.0.0.0:9101'            # /metrics для Prometheus; пусто = выключен
log_level: info
```

Запуск в консоли: `tj-agent-go --config tj-agent.yaml` (остановка Ctrl+C или
stop-файлом — graceful: события дочитаны, батч подтверждён, чекпоинт записан).
Явные CLI-флаги перекрывают значения файла; незнакомый ключ или значение вне
диапазона — понятная ошибка с именем поля.

**Служба Windows** (консоль администратора):

```powershell
tj-agent-go.exe service install --config C:\tj-agent\tj-agent.yaml
tj-agent-go.exe service start        # stop | uninstall; stop = graceful-дренаж
```

Проверка конфига без установки: `tj-agent-go service run --config <path>` в
обычной консоли. Журнал службы — `log_file` из конфига (по умолчанию
`<state_dir>\tj-agent-go.log`).

**Метрики** — endpoint `/metrics` (формат Prometheus, набор из
[storage-design.md §5](storage-design.md)): `tj_agent_lag_seconds` (главный
SLI: отставание от техжурнала), `tj_agent_events_total`,
`tj_agent_parse_errors_total`, `tj_agent_read_bytes_total` (по `collection`),
`tj_agent_files_open`, `tj_ingest_batches_total{status}`,
`tj_ingest_rows_total`, `tj_ingest_queue_depth`, `tj_ingest_insert_seconds`.
Скрейп настроен в `deploy/prometheus/prometheus.yml` (job `tj-agents`,
цель `host.docker.internal:9101` — раскомментируйте; агент должен слушать
`0.0.0.0`). Алерты по умолчанию: `tj_agent_lag_seconds > 300`,
`rate(tj_ingest_batches_total{status="failed"}[5m]) > 0` — готовые правила в
[deploy/agent/README.md](../deploy/agent/README.md).

**Как живые данные попадают на дашборды**: агент пишет в ту же `tj.events`
(rich-схема), что и офлайн-импортёр (§4), — MV наполняют роллапы `agg_*` на
вставке, дашборды §6 показывают онлайн-данные без дополнительной настройки
(time range «last 15 minutes»). Гарантии доставки: чекпоинт двигается только
после подтверждения вставки сервером — потерь нет; на краше возможны дубли
одного батча (at-least-once).
...
- Онлайн-режим (агент, §5): доставка at-least-once — на краше процесса возможны дубли одного батча (окно «вставлено, но чекпоинт не записан»); потери исключены.
